...
$ echo "console.log('Hello ' + process.env.HELLO)" > server.js

$ dotenvx run --env-file=.env.production -- node server.js
Hello production
> ^^
```
...
$ dotenvx run --env-file=.env.local --env-file=.env -- node server.js
Hello local
```
...
$ DOTENV_KEY='<dotenv_key_production>' dotenvx run -- node server.js
